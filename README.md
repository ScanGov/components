...
Outputs `/sitemap.xml`. Includes all Eleventy collection pages except those with `sitemap: false` in front matter. Emits `<lastmod>` as ISO 8601 from `modified` when available, falling back to `date`, and omits the element entirely when neither is set (search engines ignore empty or non-ISO values). Pages generated by pagination have no `date` unless the template sets one, so give those templates a `modified` value via `eleventyComputed`.
